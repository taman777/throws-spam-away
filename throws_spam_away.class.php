 ...
 * WordPress's Plugin
 * @author Takeshi Satoh@GTI Inc. 2018
 * @version 3.0
 */
class ThrowsSpamAway {
	...
	public function __construct() {
		...
		if ( $skdc < $lower_spam_keep_day_count ) {
			$skdc = $lower_spam_keep_day_count;
		}
	 ...
	 */
	function activate() {
		global $default_dummy_param_field_flg, $default_on_flg, $default_without_title_str, $default_japanese_string_min_count;
	...
	function validate_comment(
		...
		global $default_without_title_str;  // タイトル文字列は文字列カウントから排除するか　1:する
                ...
                <table class="form-table">
                    ...
                    <tr valign="top">
                        ...
                        <td><input type="text" name="tsa_caution_message" size="80"
                            ...
                            （初期設定:<?php echo $default_caution_msg; ?>）
                        </td>
                    </tr>
                    <tr valign="top">
                ...
                <table class="form-table">
                    <tr valign="top">
                        ...
                        <td><input type="text" name="tsa_error_message" size="80"
                                   value="<?php echo get_option( 'tsa_error_message', $default_error_msg ); ?>"/><br/>（初期設定:<?php echo $default_error_msg; ?>
                            ）
                        </td>
                    </tr>
                </table>
                ...
                <table class="form-table">
                    ...
                    <tr valign="top">
                        ...
                        <td><input type="text" name="tsa_url_count_over_error_message"
                            ...
                            （初期設定:<?php echo $default_url_count_over_error_msg; ?>）
                        </td>
                    </tr>
                </table>
                <a href="#option_setting" class="alignright"><?php _e( '▲ 上へ', self::DOMAIN ); ?></a>
                ...
                <table class="form-table">
                    ...
                    <tr valign="top">
                        ...
                        <td><input type="text" name="tsa_ng_key_error_message" size="80"
                            ...
                            （初期設定:<?php echo $default_ng_key_error_msg; ?>）
                        </td>
                    </tr>
                    <tr valign="top">
                    ...
                    <tr valign="top">
                        ...
                        <td><input type="text" name="tsa_must_key_error_message" size="80"
                            ...
                            （初期設定:<?php echo $default_must_key_error_msg; ?>）
                        </td>
                    </tr>
                </table>
                ...
                <table class="form-table">
                    <tr valign="top">
                        ...
                        <td><?php
                            ...
                            <label><input type="checkbox" name="tsa_spam_champuru_flg"
                                          value='1' <?php esc_attr_e( $chk ); ?> />スパムブラックリストサービスに登録されているIPアドレスからのコメントを拒否する</label><br/>
                    ...
                    <tr valign="top">
                        ...
                        <td><input type="text" name="tsa_block_ip_address_error_message"
                                   size="80"
                                   value="<?php echo get_option( 'tsa_block_ip_address_error_message', $default_block_ip_address_error_msg ); ?>"/><br/>（初期設定：<?php echo $default_block_ip_address_error_msg; ?>
                            ）
                        </td>
                    </tr>
                </table>
                    ...
                    <tr style="background-color: #fefefe;" valign="top">
                        ...
                        <td>
							...
							?> <label><input type="checkbox"
									...
									<?php esc_attr_e( $chk ); ?> />&nbsp;<?php _e( 'ホワイトリストに登録したIPアドレス以外からの投稿を無視する（ホワイトリストへの登録がない場合は有効になりません）', self::DOMAIN ); ?>
                            </label><br/>
							<?php _e( '（初期設定：', self::DOMAIN ); ?><?php echo( $default_only_whitelist_ip_flg != '1' ? __( "しない", self::DOMAIN ) : __( "する", self::DOMAIN ) ); ?><?php _e( '）', self::DOMAIN ); ?>
                            <br>
							<?php _e( '※ホワイトリストで登録したIP以外の投稿は無視されますのでこの設定は慎重に行ってください。（すべての設定より優先します）<br/>※エラーメッセージは「ブロック対象のIPアドレスからの投稿時に表示される文言」が使われます。（エラー表示時のみ）', self::DOMAIN ); ?>
                        </td>
                    </tr>
                ...
                <table class="form-table">
                    <tr valign="top">
                        ...
                        <td><?php
							...
							<?php echo sprintf( __( "※Throws SPAM Away設定画面表示時に時間がかかることがあります。<br/>※「保存する」を解除した場合でもテーブルは残りますので%d日以内の取得データは表示されます。", self::DOMAIN ), get_option( 'tsa_spam_keep_day_count', $default_spam_keep_day_count ) ); ?>
                            <br>
							<?php echo sprintf( __( "（初期設定：%s）", self::DOMAIN ), ( $default_spam_data_save != '1' ? __( "しない", self::DOMAIN ) : __( "する", self::DOMAIN ) ) ); ?>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <input
                                    ...
                                    value="<?php echo get_option( 'tsa_spam_keep_day_count', $default_spam_keep_day_count ); ?>"><?php echo sprintf( __( "日分（最低%d日）", self::DOMAIN ), $lower_spam_keep_day_count ); ?><?php echo sprintf( __( "（初期設定： %d）", self::DOMAIN ), $default_spam_keep_day_count ); ?>
                            <br>&nbsp;
							<?php
							$chk = '';
							...
							<?php echo sprintf( __( "※一度消したデータは復活出来ませんのでご注意ください。また最低%d日分は保存されます。", self::DOMAIN ), $lower_spam_keep_day_count ); ?>
                        </td>
                    </tr>
                </table>
                ...
                <table class="form-table">
                    <tr>
                        ...
                        <td><?php
							...
							<?php echo sprintf( __( "（初期設定： %s）", self::DOMAIN ), ( $default_spam_limit_flg != '1' ? __( "しない", self::DOMAIN ) : __( "する", self::DOMAIN ) ) ); ?>
                            <br>
							<?php _e( "一定時間:", self::DOMAIN ); ?><input
                                    type="text" name="tsa_spam_limit_minutes" size="3"
                                    value="<?php echo get_option( 'tsa_spam_limit_minutes', $default_spam_limit_minutes ); ?>"><?php _e( "分以内に", self:: DOMAIN ); ?>
							<?php _e( "一定回数:", self::DOMAIN ); ?><input type="text" name="tsa_spam_limit_count" size="3"
                                                                        value="<?php echo get_option( 'tsa_spam_limit_count', $default_spam_limit_count ); ?>"><?php _e( "回スパムとなったら", self::DOMAIN ); ?>
							<?php _e( "<b>次から</b>", self::DOMAIN ); ?>
							<?php _e( "一定時間:", self::DOMAIN ); ?><input type="text" name="tsa_spam_limit_over_interval"
                                                                        size="3"
                                                                        value="<?php echo get_option( 'tsa_spam_limit_over_interval', $default_spam_limit_over_interval ); ?>"><?php _e( "分間", self::DOMAIN ); ?>
                            <br/>
							<?php echo sprintf(
							        __( "（初期設定：一定時間「%d」分以内に一定回数「%d」回スパムとなったら次から「%d」分間）<br/>当該IPアドレスからのコメントを強制スパム扱いします。", self::DOMAIN ),
                                    $default_spam_limit_minutes, $default_spam_limit_count, $default_spam_limit_over_interval ); ?>
                            <br>
							...
							<?php echo sprintf( __( "（初期設定：%s）", self::DOMAIN ), $default_spam_limit_over_interval_error_msg ); ?>
                        </td>
                    </tr>
                </table>
                <a href="#option_setting" class="alignright"><?php _e( '▲ 上へ', self::DOMAIN ); ?></a>
	...
	function trackback_spam_away( $tb ) {
		...
		if ( ! $newThrowsSpamAway->ip_check( $remote_ip ) ) {
			...
			if ( ! $newThrowsSpamAway->validation( $comment, $author, $post_id ) ) {
				$tb_val['is_spam'] = true;
			} else // URL検索する場合、URL包含検査 （このブログのURLを含んでない場合エラー
	...
	function spams_list() {
        ...
        <div class="wrap">
			...
			if ( get_option( 'tsa_spam_data_save' ) == '1' ) {
				...
				if ( $gdays < $lower_spam_keep_day_count ) {
					$gdays = $lower_spam_keep_day_count;
				}
				// 表カラー
				$unique_color = "#114477";
				$web_color    = "#3377B6";
							...
							foreach ( $results as $item ) {
								...
								switch ( $spam_error_type ) {
									...
									case BLOCK_IP:
										...
										break;
									case SPAM_BLACKLIST:
										$spam_error_type_str = __( "スパムブラックリスト", self::DOMAIN );
										break;
									case SPAM_TRACKBACK:
									...
									case DUMMY_FIELD:
										...
										break;
									case NOT_IN_WHITELIST_IP:
										$spam_error_type_str = __( "ホワイトリスト許可IP以外", self::DOMAIN );
										break;
								}
								?>
                                <tr>
